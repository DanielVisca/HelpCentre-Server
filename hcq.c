...
char *print_currently_serving(Ta *ta_list) {
    ...
    char *mal_currently_serving; 

    if (ta_list == NULL) {
        asprintf(&mal_currently_serving, "No TAs are currently working.\r\n");
        return mal_currently_serving;
    }
    // The first character in currently serving will be a null terminator so that strcat works 
    char currently_serving[((sizeof(char)*83) *ta_num)]; //two names + String format * the number of TAs
    strcpy(currently_serving,"\0");
    
    while (ta_list != NULL) {
      if (ta_list->current_student != NULL) {
           char serving[sizeof(ta_list->name) + sizeof(ta_list->current_student->name) + (sizeof(char) *21)];  
           sprintf(serving, "TA=: %s is serving %s.\r\n",
                ta_list->name,
                ta_list->current_student->name);
           strcat(currently_serving, serving);
      
       } else {
           char no_students[sizeof(ta_list->name) +(sizeof(char)* 23)];
           sprintf(no_students, "TA: %s has no student\r\n", ta_list->name);
           strcat(currently_serving,no_students);
       }
       ta_list = ta_list->next;
    }
    asprintf(&mal_currently_serving,"%s", currently_serving);
    return mal_currently_serving;
}
...
char *print_full_queue(Student *stu_list) {
    ...
    char *full_queue = malloc(sizeof(Student) * stu_num *stu_num);
    //strcpy(full_queue, '\0');
    while (stu_list != NULL) {
        char printline[20 + sizeof(stu_list->name) + sizeof(stu_list->course->code)];
        sprintf(printline, "Student %s:%s\r\n",stu_list->name, stu_list->course->code);
        strcat(full_queue, printline);
